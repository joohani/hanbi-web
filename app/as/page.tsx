    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans pb-20">
      ...
      <main className="max-w-5xl mx-auto px-6 pt-16">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-4xl font-black mb-2">A/S 의뢰</h1>
            <p className="text-gray-600">제품 고장 및 유지보수에 대한 A/S를 접수해 주시면 신속하게 처리해 드립니다.</p>
          </div>
          <Link href="/as/write" className="bg-blue-700 text-white font-bold px-6 py-3 rounded hover:bg-blue-800 transition">
            + A/S 접수하기
          </Link>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                inquiries.map((inquiry) => (
                  <tr key={inquiry.id} onClick={() => handleRowClick(inquiry)} className="border-b border-gray-100 hover:bg-gray-50 transition cursor-pointer">
                    <td className="p-4 text-center">
                      {inquiry.is_answered ? (
                        <span className="bg-green-100 text-green-800 text-xs font-bold px-2 py-1 rounded">처리완료</span>
                      ) : (
                ...
                <tr><td colSpan={4} className="p-8 text-center text-gray-400">접수된 A/S 의뢰가 없습니다.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-lg shadow-xl overflow-hidden">
            <div className="bg-gray-100 p-4 border-b flex justify-between items-center">
              <h3 className="font-bold text-lg">A/S 접수 상세 확인</h3>
